...
如果目标是学习 Codex，不需要一开始复制它的 workspace。更好的练习路线是分四级。

| 阶段 | 能力 | 不做什么 |
|------|------|----------|
| v0.1 | 单线程 agent loop、2 个工具、事件日志 | 不做多 agent、不做插件、不做自动压缩 |
| v0.2 | 工具审批、patch 编辑、简单 history 裁剪 | 不做动态工具、不做远程 compact |
| v0.3 | TUI/exec 双入口、MCP client、AGENTS.md | 不做 app-server 全量 item API |
| v1.0 | compact、memory、hooks、subagents、state DB | 不追求和 Codex 完全一致，只保留必要边界 |

```mermaid
flowchart LR
    A["v0.1 loop + tools"] --> B["v0.2 safety + patch"]
    B --> C["v0.3 multi-entry + MCP"]
    C --> D["v1.0 context + hooks + memory"]
```
...
| 自己实现时的组件 | Codex 对应 |
|------------------|------------|
| `Input` / `Event` enum | `Submission` / `Op` / `EventMsg` |
| `AgentLoop` | `RegularTask` + `run_turn` |
| `ToolSpec` + handler map | `codex_tools::ToolSpec` + `ToolRegistry` |
| `ToolExecutor` | `ToolOrchestrator` + runtimes |
| `HistoryStore` | `ContextManager` + rollout |
| `ApprovalPolicy` | approval、Guardian、exec policy |
| `CompactStrategy` | `compact.rs`、`compact_remote.rs` |

这张表的用法不是照抄类名，而是照抄边界。只要边界对，语言、框架、存储都可以换。
